function o = call_cells_scClassPrior(o, gSet, BackgroundImage, iExampleCell)
% o = o.call_cells(gSet)
%  
...
if ~isempty(o.CellCallShowCenter) && o.Graphics==2
    ...
    % system
    o.plot(DapiBoundaries,[x0 x1 y0 y1]);
    change_gene_symbols(8);
    iss_show_genes([], 'Vsnl1');
    % change line width and size of dots
    ch = get(gca, 'Children');
    for gg = 1:numel(ch)-1
        ch(gg).LineWidth = 1;
        if strcmp(ch(gg).Marker, '.')
            ch(gg).MarkerSize = 15;
        end
    end
    ...
    plot([SpotYX(rn,2) , CellYX(SpotBestNeighb(rn),2)]', ...
        [SpotYX(rn,1) , CellYX(SpotBestNeighb(rn),1)]', 'Color', [.65 .65 .65], 'LineWidth', 1.5);
    
    % text to show best classes
    [BestProb, BestClass] = max(pCellClass(1:end-1,:),[],2);
    text(CellYX(iExampleCell,2), CellYX(iExampleCell,1),...
        [ClassDisplayNames(BestClass(iExampleCell)) ['p=' num2str(round(BestProb(iExampleCell), 2))]],...
    ...
    if ~isempty(o.CellCallShowCenter)
        ...
        title('iteration0', 'Color', 'w');
        set(gca, 'Clipping', 'on');
        saveas(gcf, [num2str(iExampleCell) '_iteration0.png']);
    end
end
    ...
    if ~isempty(o.CellCallShowCenter) && (Converged || o.Graphics==2 || i==o.CellCallMaxIter)
        ...
        if ~isempty(o.CellCallShowCenter)
            ...
            axis(reshape(fliplr([o.CellCallShowCenter;o.CellCallShowCenter])...
                +[-o.CellCallShowRad; o.CellCallShowRad], 1, 4));
            set(gca, 'Clipping', 'on');
            title(['iteration' num2str(i)], 'color', 'w');
            saveas(gcf, [num2str(iExampleCell) '_iteration' num2str(i) '.png']); 
        end
